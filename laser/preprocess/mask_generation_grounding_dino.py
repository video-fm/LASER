...
def generate_masks_grounding_dino(grounding_model,
    ...
    if sample_frames and num_sample_frames is not None and num_sample_frames < total_frames:
        # Uniformly sample frames
        frame_indices = np.linspace(0, total_frames - 1, num_sample_frames, dtype=int)
        print(f"Sampling {num_sample_frames} frames out of {total_frames} total frames")
    ...
    for image_id in tqdm(frame_indices, desc='Detecting objects'):
        image = video_tensor[image_id]
    ...
    assert len(frame_indices) == len(video_boxes)
    ...
    for frame_id in tqdm(frame_indices, desc='Processing frames'):
        if frame_id not in video_boxes:  # Skip if no detections in this frame
            continue
            
        bboxes = video_boxes[frame_id]
