...
def parse_args(model_name=None, epoch_num=None):
    ...
    data_dir = os.path.abspath(os.path.join(os.path.abspath(__file__), f"../../data"))
    model_dir = os.path.abspath(os.path.join(data_dir, f"LLaVA-Video-178K-v2/checkpoints"))
...
def compute_metrics(
    ...
    for ((oid, name), p) in cate_pred.items():
        if oid not in new_cate_pred:
            new_cate_pred[oid] = []
        new_cate_pred[oid].append((float(p), name))  # Convert JAX array to float
    ...
    for ((fid, (from_id, to_id), rel), p) in binary_pred.items():
        ...
        new_binary_pred[(fid, from_id, to_id)].append((float(p), rel))  # Convert JAX array to float
...
class Tester():
    def __init__(self, 
        ...
        # Load CLIP resources at class level first
        PredicateModelJAX.load_clip_resources(clip_model_name)
        
        # Setting up the JAX CLIP model
        if load_model and model_dir is not None and os.path.exists(model_dir) and len(os.listdir(model_dir)) > 0:
            ...
            model_path = os.path.join(model_dir, f"{model_name}.{latest_model_id}.model")
            model_info = torch.load(model_path)

            # Initialize model and create train state only once
            self.predicate_model = PredicateModelJAX(
                hidden_dim=0,
                num_top_pairs=test_num_top_pairs,
                model_name=clip_model_name
            )

            # Create train state with default parameters
            self.train_state = create_train_state(
                self.predicate_model,
                self.rng,
                learning_rate=1e-4,
                dummy_data=None  # Will be created by create_train_state
            )

            # Convert PyTorch parameters to JAX
            if isinstance(model_info, dict):
                # If model_info is already a state dict
                pt_state_dict = model_info
            else:
                # If model_info is a PyTorch model
                pt_state_dict = model_info.state_dict()

            # Initialize with default parameters first
            jax_params = self.train_state.params

            # Update CLIP model parameters from PyTorch state dict
            for model_type in ['cate', 'unary', 'binary']:
                if f'clip_{model_type}_model.state_dict' in pt_state_dict:
                    clip_params = pt_state_dict[f'clip_{model_type}_model.state_dict']
                    # Convert PyTorch parameters to JAX arrays
                    for k, v in clip_params.items():
                        if isinstance(v, torch.Tensor):
                            jax_params[model_type][k] = jnp.array(v.detach().cpu().numpy())

            # Update train state with converted parameters
            self.train_state = self.train_state.replace(params=jax_params)
            ...
            # Create train state for new model
            self.train_state = create_train_state(
                self.predicate_model,
                self.rng,
                learning_rate=1e-4,
        ...
        # Apply function    
        self._apply_fn = self.predicate_model.apply 
    ...
    def eval_video(self,
        ...
        # Convert numpy arrays to JAX arrays where needed
        batched_reshaped_raw_videos = jnp.array(batched_reshaped_raw_videos)
        batched_gt_masks = jnp.array(batched_gt_masks)
        ...
        # Convert object pairs to proper format
        batched_gt_object_pairs = [(int(v), int(f), (int(s), int(t))) for v, f, (s, t) in batched_gt_object_pairs]

        # Use the model's apply function directly without JIT
        outputs = self._apply_fn(
            ...
            batched_obj_pairs=batched_gt_object_pairs,
        ...
        # Unpack the outputs
        batched_image_cate_probs, batched_image_unary_probs, batched_image_binary_probs, dummy_prob = outputs
    ...
    def eval(self, recall_thres_ls=[1, 5, 10], precision_thres_ls=[1, 5, 10]):
        ...
        for ct, dp_list in enumerate(iterator):
            ...
            if len(result.keys()) == 0:
                ...
                # Call the eval_video function with both threshold lists
            ...
            report = {
                ...
                'processed_vids': total_metrics_res['processed_vids'],
                'precision': {'cate': {}, 'binary': {}},
                'recall': {'cate': {}, 'binary': {}}
            }
...
def main():
    ...
    data_args = {
        ...
        "device": "cpu",  # JAX will handle device placement
